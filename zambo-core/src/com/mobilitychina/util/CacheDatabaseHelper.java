...
public class CacheDatabaseHelper {
	...
	public CacheDatabaseHelper(String cachePath, String cacheName) {
		super();
	...
	public Object get(String key) {
		...
		try {
			...
			sql.append("SELECT V FROM ").append(this.cacheName).append(" WHERE K=\"").append(key).append("\"");
	 ...
	 * @return 缓存数据
	 */
	public Object get(String key, boolean shouldCacheTime) {
		if (this.db == null || !this.db.isOpen() || key == null)
			return null;
		if (!shouldCacheTime) {
			return this.get(key);
	...
	public boolean put(String key, Object value) {
		...
		if (this.get(key) == null) { // 插入一条新数据
			...
			try {
				...
				insertHelper.bind(insertHelper.getColumnIndex("V"), (byte[]) value);
