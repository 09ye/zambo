...
*/
	...
	/*public static SoapTask getAllZamboCustomersTask(Context context, String type) {
		...
		try {
			message.put("type", "basic");
			message.put("name", UserInfoManager.getInstance().getUserId());
	...
	public static SoapTask getPlanlistTask(String startDay, String endDay) {
		...
		params.add(new BasicNameValuePair("arg2", UserInfoManager.getInstance().getUserId()));
	...
	public static SoapTask insertSiemensUpload(String report, String detail, String share, String dateLineId,
		...
		params.add(new BasicNameValuePair("arg6", UserInfoManager.getInstance().getUserId()));
...
//		task.setUrl("http://192.168.11.195:8091/create_visit_plan");
		...
		try {
			...
			json.put("identication", message);
			data.put("emp_id" , "3");
			data.put("cust_id", custIds);
			data.put("plan_status", "A");
			data.put("plan_type", type);
			data.put("plan_visit_date", date);
			json.put("data", data);
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
